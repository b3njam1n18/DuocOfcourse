...
export default function ClaseProfesor() {
  ...
  return (
    <section className="p-8">
      ...
      <div className="relative bg-white border rounded-md p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-[240px,1fr] gap-4">
          ...
          <div className="bg-gray-200 p-4 rounded">
            ...
            <p className="text-sm mt-2">{video.descripcion}</p>
          </div>
        </div>
        <GearButton onClick={() => setOpenEditVideo(true)} className="absolute right-[-22px] top-1/2 -translate-y-1/2" />
      </div>
      ...
      <div className="mt-6 bg-white border rounded-md p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          >
            ...
            <span className="font-semibold text-duocceleste">AGREGAR VIDEO</span>
          </button>
      ...
      {/* Modals de edición */}
      <EditarVideoModal open={openEditVideo} onClose={() => setOpenEditVideo(false)} onSubmit={guardarVideoEdit} initial={video} />
      <EditarQuizModal open={openEditQuiz} onClose={() => setOpenEditQuiz(false)} onSubmit={guardarQuizEdit} initial={quiz} />

      {/* ✅ Modals de creación */}
      <AgregarVideoModal open={openNewVideo} onClose={() => setOpenNewVideo(false)} onSubmit={crearVideo} />
