...
namespace Api.Repository.Repositories
{
    public class EvaluationsRepository : IEvaluationRepository
    {
        ...
        public async Task<List<Evaluations>> GetEvaluationsByLessonAsync(
        long courseId,
        long lessonId,
        bool? published)
            {
                // 1) Partimos con un IQueryable normal
                IQueryable<Evaluations> query = _context.Evaluations
                    .Where(e => e.CourseId == courseId && e.LessonId == lessonId);

                // 2) Aplicamos el filtro opcional published
                if (published.HasValue)
                {
                    query = query.Where(e => e.IsPublished == published.Value);
                }

                // 3) Recién aquí aplicamos los Include / ThenInclude
                return await query
                    .Include(e => e.Questions)
                        .ThenInclude(q => q.Options)
                    .OrderBy(e => e.DueAt ?? DateTime.MaxValue)
        ...
        public async Task<Attempts?> GetLastAttemptByStudentAsync(long evaluationId, long studentId)
        {
            return await _context.Attempts
                .Include(a => a.Evaluation)
                .Include(a => a.Answers)
                    .ThenInclude(ans => ans.Question)
                        .ThenInclude(q => q.Options)
                .Where(a => a.EvaluationId == evaluationId && a.StudentId == studentId)
